    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBConnection.getConnection()) {
            ...
            try (PreparedStatement check = conn.prepareStatement("SELECT password FROM users WHERE email=?")) {
                ...
                try (ResultSet rs = check.executeQuery()) {
                    if (!rs.next() || !PasswordUtils.matches(oldPassword, rs.getString("password"))) {
                        response.sendRedirect("user_profile.jsp?error=wrongpass");
                        return;
                    }
                }
            }
            ...
            if (updatePassword || !sessionEmail.equalsIgnoreCase(email)) {
                TicketNotificationService.notifyAccountUpdated(sessionEmail, email, "user");
            }
            response.sendRedirect("user_profile.jsp?success=1");
        } catch (IllegalArgumentException e) {
            response.sendRedirect("user_profile.jsp?error=validation");
        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect("user_profile.jsp?error=db");
        }
    }
}
