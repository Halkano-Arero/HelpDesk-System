...
@WebServlet("/UserInsertTicketServlet")
public class UserInsertTicketServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session == null || !"user".equalsIgnoreCase(String.valueOf(session.getAttribute("role")))) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (ps.executeUpdate() == 0) {
                response.sendRedirect(request.getContextPath() + "/user_create_ticket.jsp?error=insert_failed");
                return;
            }
            ...
            response.sendRedirect(request.getContextPath() + "/UserDashboardServlet?msg=ticket_created");
        } catch (IllegalArgumentException e) {
            response.sendRedirect(request.getContextPath() + "/user_create_ticket.jsp?error=validation");
        } catch (Exception ex) {
            ex.printStackTrace();
            response.sendRedirect(request.getContextPath() + "/user_create_ticket.jsp?error=exception");
        }
    }
